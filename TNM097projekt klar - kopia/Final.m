%% Load and crop the original image and decide size of the pearls
clear all
clc
...
img = im2double(imread('images/cat.jpg'));
img = checkSize(img);
[row, col] = size(img(:,:,1));

p = 40; % number of pearls along x-axis
s = round(row/p);
...
color_space = OptimizeColors(unique_colors, similarity_threshold);
%showRGB(da);
...
blabs  = GenPearls(blabshape,color_space);
blibs = GenPearls(blibshape,color_space);
blobs = GenPearls(blobshape,color_space);

%For the other two shapes, simply rotate 
r_blabs = GenPearls(imrotate(blabshape,90),color_space);
r_blibs = GenPearls(imrotate(blibshape,90),color_space);
...
scaleBlb = s/r; %Scale factor for the pearls

[nr_colors e] = size(color_space);
for i=1:nr_colors
